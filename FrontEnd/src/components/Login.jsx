...
const Login = ({ handleRedirect, setShowLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true)
      await fetch('http://localhost:4000/api/sessions/login', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className='w-full h-screen bg-stone-700 bg-opacity-50 flex justify-center items-center absolute top-0'>
      <form onSubmit={handleSubmit} ref={formRef} className='w-[45%] min-h-[90vh] rounded-2xl bg-white flex flex-col justify-center items-center'>
        <div className='w-full flex h-[10%] pt-2 px-3 gap-8 items-center'>
          <svg onClick={() => setShowLogin(false)} className='w-8 h-8 rounded-full cursor-pointer hover:bg-gray-200 p-2' viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M5 5L19 19M5 19L19 5" stroke="#000000" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path> </g></svg>
        ...
        <div className='w-[55%] px-5 pt-5 flex text-center flex-col justify-center items-center h-[90%] overflow-y-auto gap-2'>
          ...
          <input className='border py-3 px-2 w-full rounded-md' name='email' type="email" placeholder='Email' />
          <input className='border py-3 px-2 w-full rounded-md' name='password' type="password" placeholder='Password' />
          <button className='w-full border border-gray-300 rounded-full h-10 mt-3 bg-black text-white font-semibold py-1'>Login</button>
          <button className='w-full border border-gray-300 rounded-full h-10 mt-2 font-semibold hover:bg-gray-100 duration-200 py-1'>Forgot password?</button>
          <p className='mt-3 mb-5 text-gray-600'>Don't have an account? <button onClick={() => handleRedirect()} className='text-[#1D9BF0] hover:underline cursor-text'>Sign up</button></p>
        </div>
      </form>
    </div>
  )
